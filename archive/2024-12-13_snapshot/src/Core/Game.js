...
export class Game {
    ...
    updateWaypointCurve() {
        ...
        // Create Catmull-Rom curve through ALL control points (for visualization)
        // Use 'centripetal' for smoother curves without sharp corners
        const curve = new THREE.CatmullRomCurve3(unit.waypointControlPoints, unit.isPathClosed, 'centripetal', 0.5);
        
        // Sample curve for visualization (dense points)
        // CRITICAL: For closed paths, we need VERY high density to ensure
        // the closing segment (last->first) is properly projected to terrain
        const baseSamples = unit.waypointControlPoints.length * 30;
        const numSamples = Math.max(200, unit.isPathClosed ? baseSamples * 5 : baseSamples);
        const curvePoints = curve.getPoints(numSamples);
        
        // Project curve points onto terrain surface
        // Use unit's groundOffset for consistent height with manual steering
        const groundOffset = unit.groundOffset || 0.5;
        const projectedPoints = curvePoints.map(p => {
        ...
        // Create/update THICK curve visualization using TubeGeometry (per-unit)
        if (unit.waypointCurveLine) {
        ...
        const tubeGeo = new THREE.TubeGeometry(projectedCurve, numSamples, 0.08, 8, unit.isPathClosed);
        const tubeMat = new THREE.MeshBasicMaterial({ 
            color: unit.isPathClosed ? 0x00ff88 : 0x00cc66, // Bright Green (Closed) vs Medium Green (Open)
            transparent: true,
        ...
        // PATH SYNC: Set unit.path to the FULL projected curve
        // SIMPLE PATH RESUME (matches simple sequential path following in Unit.js)
        const newCPCount = unit.waypointControlPoints.length;
        
        if (newCPCount >= 2) {
            const oldCPCount = unit.lastCommittedControlPointCount || 0;
            const oldPathIndex = unit.pathIndex || 0;
            
            // Set the full path
            unit.path = projectedPoints.map(p => p.clone());
            
            // SIMPLE RESUME LOGIC - ALWAYS find closest point, NEVER go backwards
            // When path changes for ANY reason, find where unit currently is on the new path
            let closestIdx = 0;
            ...
            // SNAP unit to the closest path point to prevent underground movement
            // Then target the NEXT point (always forward)
            if (bestDist > 0.5) {
                // Unit is far from path - snap to closest point first
                unit.position.copy(unit.path[closestIdx]);
                console.log(`PATH SNAP: Unit was ${bestDist.toFixed(2)} away, snapped to point ${closestIdx}`);
            }
            
            // Target NEXT point (forward only)
            let targetIdx = closestIdx + 1;
            if (targetIdx >= unit.path.length) {
                if (unit.loopingEnabled || unit.isPathClosed) {
                    targetIdx = 0; // Wrap around for closed paths
                } else {
            ...
            console.log(`PATH UPDATE: Closest=${closestIdx}, Target=${targetIdx}, Distance=${bestDist.toFixed(2)}`);
            ...
            console.log(`PATH SYNC: pathIndex=${unit.pathIndex}, totalPoints=${unit.path.length}, looping=${unit.loopingEnabled}`);
        }
    }
